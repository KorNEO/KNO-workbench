...
- **형태부 2차완료 항목의 형태부는 잠금**(`morphLocked`/서버 거부, 관리자만 정정). 저장은 부·차수별(`saveWrite` payload `part`), 상태·일시도 그 부만. 주차 필터는 형태부 주차+의미부 주차(대상만), 상태 필터 기준 부는 주차 선택 시 그 주차의 부, 미선택 시 작업자=작업자 단계·검수자/관리자=검수자 단계. 상세: `docs/집필_워크벤치_작업_일지_20260922.md`.
